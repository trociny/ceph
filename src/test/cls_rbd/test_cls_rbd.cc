...
TEST_F(TestClsRbd, mirror_snapshot) {
  ...
  ASSERT_EQ(2U, sn->mirror_peers.size());
  ASSERT_EQ(1U, sn->mirror_peers.count("peer1"));
  ASSERT_EQ(1U, sn->mirror_peers.count("peer2"));
  ...
  ASSERT_NE(nullptr, sn);
  ASSERT_EQ(primary, *sn);
  ASSERT_EQ(1U, sn->mirror_peers.size());
  ASSERT_EQ(1U, sn->mirror_peers.count("peer2"));

  ASSERT_EQ(-EBUSY, mirror_image_snapshot_unlink_peer(&ioctx, oid, 1, "peer2"));
  ASSERT_EQ(0, snapshot_get(&ioctx, oid, 1, &snap));
  sn = boost::get<cls::rbd::MirrorPrimarySnapshotNamespace>(
    &snap.snapshot_namespace);
  ASSERT_NE(nullptr, sn);
  ...
  ASSERT_EQ(nsn->copy_progress, 10);
