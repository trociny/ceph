...
public:
  ...
  void expect_get_copy_snap_name(librbd::MockImageCtx &mock_image_ctx) {
    EXPECT_CALL(mock_image_ctx, get_snap_name(_, _))
      .WillOnce(DoAll(SetArgPointee<1>("copy"),
                      Return(0)));
  }
...
TEST_F(TestMockDeepCopyRequest, SimpleCopy) {
  ...
  expect_get_copy_snap_name(mock_src_image_ctx);

  InSequence seq;
  expect_copy_snapshots(mock_snapshot_copy_request, 0);
  expect_copy_image(mock_image_copy_request, 0);
  if (m_src_image_ctx->snap_id != CEPH_NOSNAP &&
      (m_dst_image_ctx->features & RBD_FEATURE_OBJECT_MAP) != 0) {
  ...
  auto request = librbd::DeepCopyRequest<librbd::MockTestImageCtx>::create(
      ...
      boost::none, m_work_queue, &no_op, &ctx);
  request->send();
