...
public:
  ...
  void expect_set_size(librbd::MockTestImageCtx &mock_src_image_ctx,
                       librbd::MockTestImageCtx &mock_dst_image_ctx) {
    EXPECT_CALL(mock_src_image_ctx, get_image_size(CEPH_NOSNAP))
      .WillOnce(Return(mock_src_image_ctx.image_ctx->size));
    EXPECT_CALL(mock_dst_image_ctx, get_image_size(CEPH_NOSNAP))
      .WillOnce(Return(mock_dst_image_ctx.image_ctx->size));
  }
...
TEST_F(TestMockDeepCopyRequest, SimpleCopy) {
  ...
  expect_set_size(mock_src_image_ctx, mock_dst_image_ctx);
