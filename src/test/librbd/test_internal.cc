...
struct TestCopyDeep : public TestInternal {
  void SetUp() override {
    TestInternal::SetUp();
  ...
  void TearDown() override {
    if (is_feature_enabled(RBD_FEATURE_LAYERING | RBD_FEATURE_EXCLUSIVE_LOCK)) { // XXXMG
      deep_copy();
      compare();
  ...
  void deep_copy() {
    ...
    librbd::NoOpProgressContext no_op;
    ASSERT_EQ(0, librbd::copy_deep(m_src_ictx, m_src_ictx->md_ctx,
                                   dst_name.c_str(), m_opts, no_op));
    ASSERT_EQ(0, librbd::snap_set(m_src_ictx, cls::rbd::UserSnapshotNamespace(),
                                  nullptr));
...
TEST_F(TestCopyDeep, Empty)
{
}

TEST_F(TestCopyDeep, NoSnaps)
{
  test_no_snaps();
}

TEST_F(TestCopyDeep, Snaps)
{
  test_snaps();
}

TEST_F(TestCopyDeep, SnapDiscard)
{
  test_snap_discard();
}

TEST_F(TestCopyDeep, Stress)
{
  test_stress();
}

TEST_F(TestCopyDeep, NoSnaps_LargerDstObjSize)
{
...
TEST_F(TestCopyDeep, Snaps_LargerDstObjSize)
{
  uint64_t order = m_src_ictx->order + 1;
  ASSERT_EQ(0, m_opts.set(RBD_IMAGE_OPTION_ORDER, order));
...
TEST_F(TestCopyDeep, Stress_LargerDstObjSize)
{
  uint64_t order = m_src_ictx->order + 1;
  ASSERT_EQ(0, m_opts.set(RBD_IMAGE_OPTION_ORDER, order));
...
TEST_F(TestCopyDeep, NoSnaps_SmallerDstObjSize)
{
  uint64_t order = m_src_ictx->order - 1;
  ASSERT_EQ(0, m_opts.set(RBD_IMAGE_OPTION_ORDER, order));
...
TEST_F(TestCopyDeep, Snaps_SmallerDstObjSize)
{
  uint64_t order = m_src_ictx->order - 1;
  ASSERT_EQ(0, m_opts.set(RBD_IMAGE_OPTION_ORDER, order));
  uint64_t stripe_unit = m_src_ictx->stripe_unit >> 1;
...
TEST_F(TestCopyDeep, Stress_SmallerDstObjSize)
{
  uint64_t order = m_src_ictx->order - 1;
  ASSERT_EQ(0, m_opts.set(RBD_IMAGE_OPTION_ORDER, order));
  uint64_t stripe_unit = m_src_ictx->stripe_unit >> 1;
