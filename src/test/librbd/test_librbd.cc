...
TEST_F(TestLibRBD, MigrateLive) {
  ...
  int order = 0;
  std::string src = get_temp_image_name();
  uint64_t size = 10 << 20;

  ASSERT_EQ(0, create_image(ioctx, src.c_str(), size, &order));

  {
    rbd_image_t image;
    ASSERT_EQ(0, rbd_open(ioctx, src.c_str(), &image, nullptr));
    BOOST_SCOPE_EXIT(&image) {
      EXPECT_EQ(0, rbd_close(image));
    } BOOST_SCOPE_EXIT_END;
  ...
  std::string dst = get_temp_image_name();

  thread user([this, &ioctx, &dst, &test_data]() {
      char zero_data[TEST_IO_SIZE + 1];
      memset(zero_data, 0, sizeof(zero_data));
      ...
      int r;
      for (int i = 0; i < 100; i++) {
        r = rbd_open(ioctx, dst.c_str(), &image, nullptr);
        if (r != -ENOENT) {
          break;
        }
        usleep(100000);
  ...
  ASSERT_EQ(0, rbd_migrate(ioctx, src.c_str(), ioctx, dst.c_str(),
                           image_options));
