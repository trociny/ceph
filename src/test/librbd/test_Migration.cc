...
TEST_F(TestMigration, CloneV1ParentAbortAfterReattachParent)
{
  const uint32_t CLONE_FORMAT = 1;
  test_migrate_parent(
...
TEST_F(TestMigration, CloneV2ParentAbortAfterReattachParent)
{
  const uint32_t CLONE_FORMAT = 2;
  test_migrate_parent(
      CLONE_FORMAT, [this](librbd::ImageCtx *child_ictx) {
...
TEST_F(TestMigration, CloneV1ParentAbortAfterIncompleteReattachChild)
{
  const uint32_t CLONE_FORMAT = 1;
  test_migrate_parent(
      CLONE_FORMAT, [this](librbd::ImageCtx *child_ictx) {
           ...
           auto req1 = librbd::image::AttachChildRequest<>::create(
               child_ictx, m_ictx, m_ictx->snaps[0], nullptr, 0, CLONE_FORMAT,
               &cond1);
           req1->send();
...
TEST_F(TestMigration, CloneV2ParentAbortAfterIncompleteReattachChild)
{
  const uint32_t CLONE_FORMAT = 2;
  test_migrate_parent(
      CLONE_FORMAT, [this](librbd::ImageCtx *child_ictx) {
...
TEST_F(TestMigration, CloneV1ParentAbortAfterReattachChild)
{
  const uint32_t CLONE_FORMAT = 1;
  test_migrate_parent(
      CLONE_FORMAT, [this](librbd::ImageCtx *child_ictx) {
           ...
           C_SaferCond cond1;
           auto req1 = librbd::image::AttachChildRequest<>::create(
               child_ictx, m_ictx, m_ictx->snaps[0], src_ictx,
               src_ictx->snaps[0], CLONE_FORMAT, &cond1);
           req1->send();
           ASSERT_EQ(0, cond1.wait());
...
TEST_F(TestMigration, CloneV2ParentAbortAfterReattachChild)
{
  const uint32_t CLONE_FORMAT = 2;
  test_migrate_parent(
      CLONE_FORMAT, [this](librbd::ImageCtx *child_ictx) {
