              ...
              interval_set<uint64_t> *what)
{
  ...
  std::cout << __func__ << ": num_ops=" << num_ops << ", max_size=" << max_size << ", object_size=" << object_size << ", image_size=" << image_ctx->size << std::endl;
...
public:
  ...
  int create_snap(const char* snap_name) {
    ...
    MockObjectCopyRequest::SnapIds dst_snap_ids({dst_snap_id});
    if (!m_snap_map.empty()) {
...
TEST_F(TestMockDeepCopyObjectCopyRequest, Write) {
  ...
  expect_get_object_name(mock_src_image_ctx);
...
TEST_F(TestMockDeepCopyObjectCopyRequest, ReadMissingStaleSnapSet) {
  ...
  expect_start_op(mock_exclusive_lock);
  expect_update_object_map(mock_dst_image_ctx, mock_object_map,
                           m_dst_snap_ids[3], OBJECT_EXISTS_CLEAN, 0);
...
TEST_F(TestMockDeepCopyObjectCopyRequest, WriteSnaps) {
  ...
  expect_start_op(mock_exclusive_lock);
  expect_update_object_map(mock_dst_image_ctx, mock_object_map,
                           m_dst_snap_ids[2], OBJECT_EXISTS_CLEAN, 0);
...
TEST_F(TestMockDeepCopyObjectCopyRequest, Remove) {
  ...
  expect_start_op(mock_exclusive_lock);
  expect_update_object_map(mock_dst_image_ctx, mock_object_map,
                           m_dst_snap_ids[1], OBJECT_EXISTS_CLEAN, 0);
