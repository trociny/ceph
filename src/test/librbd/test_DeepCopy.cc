...
struct TestDeepCopy : public TestFixture {
  ...
  void deep_copy() {
    EXPECT_EQ(0, librbd::flush(m_src_ictx));
    EXPECT_EQ(0, snap_create(*m_src_ictx, "copy"));
    ...
    EXPECT_EQ(0, librbd::snap_set(m_src_ictx, cls::rbd::UserSnapshotNamespace(),
                                  nullptr));
    EXPECT_EQ(0, m_src_ictx->operations->snap_remove(
    ...
    EXPECT_EQ(0, open_image(dst_name, &m_dst_ictx));
    {
      ...
      EXPECT_EQ(CEPH_NOSNAP, m_dst_ictx->get_snap_id(
                  cls::rbd::UserSnapshotNamespace(), "copy"));
    }
  }
  ...
  void test_stress() {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      if (rand() % 2) {
        librbd::NoOpProgressContext no_op;
        uint64_t new_size =  size + rand() % size;
