...
struct MirrorEntity {
  ...
  MirrorEntity(MirrorEntityType type, const std::string &global_id, size_t size)
    : type(type), global_id(global_id), size(size) {
  }

  inline bool operator==(const MirrorEntity &rhs) const {
    return type == rhs.type && global_id == rhs.global_id && size == rhs.size;
  }
  inline bool operator<(const MirrorEntity &rhs) const {
    if (type != rhs.type) {
