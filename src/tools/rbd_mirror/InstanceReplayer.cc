...
void InstanceReplayer<I>::acquire_image(const std::string &global_image_id,
  ...
  ImageReplayer<>::RemoteImages remote_images;
  for (auto &peer : peers) {
    remote_images.insert(ImageReplayer<>::RemoteImage(
                           peer.mirror_uuid, peer.image_id, peer.io_ctx));
  }
...
template <typename I>
void InstanceReplayer<I>::start_image_replayer(ImageReplayer<I> *image_replayer) {
...
void InstanceReplayer<I>::stop_image_replayer(ImageReplayer<I> *image_replayer,
  ...
  dout(20) << image_replayer << " global_image_id="
           << image_replayer->get_global_image_id() << dendl;
