 ...
 *
 * Copyright (C) 2015 Kylin Corporation
...
public:
  ...
  int recv(NbdReq **req_) {
    ...
    ss << std::hex << ntohll(*((uint64_t *)request.handle));
    req->id = ss.str();
...
int NbdDrv::list(std::list<std::string> &devs) {
  ...
  if (socketpair(AF_UNIX, SOCK_STREAM, 0, fd) == -1) {
    int r = -errno;
    std::cerr << "rbd-nbd: socketpair failed: " << cpp_strerror(r) << std::endl;
    return r;
  }

  for (int i = 0; ; i++) {
    std::string path = "/dev/nbd" + stringify(i);
    int nbd = open(path.c_str(), O_RDWR);
    if (nbd < 0) {
      break;
    }
