...
static int do_commit(librados::IoCtx& io_ctx, const std::string &image_name,
  ...
  if (!children.empty()) {
    std::cerr << "rbd: image has childern: " << std::endl;
    ...
    std::cerr << "If open readonly a child will not detect parent update."
              << std::endl;
    if (force) {
      std::cerr << "Proceeding anyway due to force flag set." << std::endl;
    } else {
      std::cerr << "Make sure no children opened readonly or reload them\n"
                << "and run again with force flag set." << std::endl;
      return -EBUSY;
    }
  }
