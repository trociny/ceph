...
template <typename I>
void DeepCopyRequest<I>::send_set_size() {
  uint64_t size;
  {
  ...
  if (m_dst_image_ctx->get_image_size(CEPH_NOSNAP) == size) {
  ...
  ldout(m_cct, 20) << size << dendl;

  librados::ObjectWriteOperation op;
  librbd::cls_client::set_size(&op, size);
  ...
  auto ctx = new FunctionContext([this, size, finish_op_ctx](int r) {
      handle_set_size(size, r);
...
template <typename I>
void DeepCopyRequest<I>::handle_set_size(uint64_t size, int r) {
  ldout(m_cct, 20) << "r=" << r << dendl;
