...
void DeepCopyRequest<I>::send_remove_copy_snapshot() {
  ...
  Context *ctx = create_context_callback<
    DeepCopyRequest<I>, &DeepCopyRequest<I>::handle_remove_copy_snapshot>(this);

  auto req = operation::SnapshotRemoveRequest<I>::create(
      *m_dst_image_ctx, cls::rbd::UserSnapshotNamespace(), "", m_copy_snap_id,
      ctx);
  req->send();
}

template <typename I>
void DeepCopyRequest<I>::handle_remove_copy_snapshot(int r) {
  ldout(m_cct, 20) << "r=" << r << dendl;
