...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::image_copy::SnapshotCreateRequest: " \
                           << this << " " << __func__ << ": "
...
void SnapshotCreateRequest<I>::send_set_size() {
  ...
  if (m_dst_image_ctx->size == m_size) {
  ...
  int r = m_dst_image_ctx->md_ctx.aio_operate(m_dst_image_ctx->header_oid,
                                                comp, &op);
  assert(r == 0);
  comp->release();
}
...
void SnapshotCreateRequest<I>::handle_set_size(int r) {
  ...
  if (r < 0) {
    lderr(m_cct) << "failed to update image size '" << m_snap_name << "': "
...
void SnapshotCreateRequest<I>::send_remove_parent() {
  m_dst_image_ctx->parent_lock.get_read();
  if (m_dst_image_ctx->parent_md.spec.pool_id == -1 ||
      m_dst_image_ctx->parent_md.spec == m_parent_spec) {
    m_dst_image_ctx->parent_lock.put_read();
    send_set_parent();
    return;
  }
  m_dst_image_ctx->parent_lock.put_read();
...
void SnapshotCreateRequest<I>::handle_remove_parent(int r) {
  ...
  if (r < 0) {
    lderr(m_cct) << "failed to remove parent '" << m_snap_name << "': "
                 << cpp_strerror(r) << dendl;
    finish(r);
    return;
  }

  {
    ...
    RWLock::WLocker parent_locker(m_dst_image_ctx->parent_lock);
    m_dst_image_ctx->parent_md.spec = {};
    m_dst_image_ctx->parent_md.overlap = 0;
  }
...
void SnapshotCreateRequest<I>::send_set_parent() {
  m_dst_image_ctx->parent_lock.get_read();
  if (m_dst_image_ctx->parent_md.spec == m_parent_spec &&
      m_dst_image_ctx->parent_md.overlap == m_parent_overlap) {
    m_dst_image_ctx->parent_lock.put_read();
    send_snap_create();
    return;
  }
...
void SnapshotCreateRequest<I>::handle_set_parent(int r) {
  ...
  if (r < 0) {
    lderr(m_cct) << "failed to set parent '" << m_snap_name << "': "
                 << cpp_strerror(r) << dendl;
    finish(r);
    return;
  }
...
template <typename I>
Context *SnapshotCreateRequest<I>::start_dst_op() {
  RWLock::RLocker owner_locker(m_dst_image_ctx->owner_lock);
...
template class librbd::image_copy::SnapshotCreateRequest<librbd::ImageCtx>;
