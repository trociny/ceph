...
public:
  static DisableFeaturesRequest *create(ImageCtxT &image_ctx, Context *on_finish,
                                       uint64_t features) {
    return new DisableFeaturesRequest(image_ctx, on_finish, features);
  }
...
private:
   ...
   *    |                    required)
   *    v
   * STATE_REMOVE_MIRROR_IMAGE (skip if not
   *    |                       required)
   *    v
   * STATE_CLOSE_JOURNAL (skip if not
   *    |                 required)
   *    v
   * STATE_REMOVE_JOURNAL (skip if not
   *    |                 required)
   *    v
   * STATE_SET_FEATURES
   *    |
