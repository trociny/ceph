...
template <typename I>
void EnableFeaturesRequest<I>::send_disable_mirror_image() {
  I &image_ctx = this->m_image_ctx;
  CephContext *cct = image_ctx.cct;

  if (!m_enable_mirroring || m_mirror_mode == cls::rbd::MIRROR_MODE_POOL) {
    send_create_journal();
    return;
  }
  ...
  Context *ctx = create_context_callback<
    EnableFeaturesRequest<I>,
    &EnableFeaturesRequest<I>::handle_disable_mirror_image>(this);

  auto req = mirror::DisableRequest<I>::create(&image_ctx, true, true, ctx);
  req->send();
}

template <typename I>
Context *EnableFeaturesRequest<I>::handle_disable_mirror_image(int *result) {
  I &image_ctx = this->m_image_ctx;
  CephContext *cct = image_ctx.cct;
  ldout(cct, 20) << this << " " << __func__ << ": r=" << *result << dendl;
