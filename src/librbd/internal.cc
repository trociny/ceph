...
int mirror_image_enable_internal(ImageCtx *ictx) {
  ...
  image::MirrorEnableRequest<ImageCtx> *req =
    image::MirrorEnableRequest<ImageCtx>::create(ictx, &cond);
  req->send();
...
int mirror_image_disable_internal(ImageCtx *ictx, bool force,
  ...
  image::MirrorDisableRequest<ImageCtx> *req =
    image::MirrorDisableRequest<ImageCtx>::create(ictx, force, remove, &cond);
  req->send();
...
#define READ_SIZE 4096
  ...
  {
    return ictx->operations->update_features(features, enabled);
  }
