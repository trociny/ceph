...
void ObjectCopyRequest<I>::send_list_snaps() {
  ...
  auto object_number = m_src_object_extents.rbegin()->first;
...
void ObjectCopyRequest<I>::handle_list_snaps(int r) {
  ...
  m_src_object_extents.erase(std::prev(m_src_object_extents.end()));
  if (m_src_object_extents.empty()) {
...
template <typename I>
void ObjectCopyRequest<I>::init_extents() {
  m_snap_copy_ops = {};
  ...
  for (auto &e : image_extents) {
    ...
    for (auto &p : src_object_extents) {
      for (auto &s : p.second) {
        m_src_object_extents[s.objectno].push_back({s.offset, s.length});
      }
    }
  }

  assert(!m_src_object_extents.empty());

  ldout(m_cct, 20) << m_src_object_extents.size() << " src objects, first: "
                   << *m_src_object_extents.begin() << dendl;
}

template <typename I>
void ObjectCopyRequest<I>::compute_diffs() {
  auto iter = m_src_object_extents.rbegin();
  assert(iter != m_src_object_extents.rend());
  auto src_object_number = iter->first;
  bool first_src_object = (m_src_object_extents.size() == 1);

  for (auto &it : iter->second) {
    auto src_object_off = it.first;
    auto len = it.second;

    interval_set<uint64_t> copy_interval;
    copy_interval.insert(src_object_off, len);
