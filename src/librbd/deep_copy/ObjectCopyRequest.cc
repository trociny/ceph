...
void ObjectCopyRequest<I>::init_offsets() {
  ...
  Striper::extent_to_file(m_cct, &m_dst_image_ctx->layout, m_dst_object_number,
                          0, m_dst_image_ctx->layout.object_size, file_extents);

  for (auto &e : file_extents) {
    ...
    for (auto &p : src_object_extents) {
      for (auto &s : p.second) {
        m_src_object_offsets[s.objectno].push_back({s.offset, s.length});
      }
    }
  }
...
void ObjectCopyRequest<I>::compute_diffs() {
  auto iter = m_src_object_offsets.rbegin();
  assert(iter != m_src_object_offsets.rend());
