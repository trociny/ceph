...
template<typename I>
ListWatchersRequest<I>::ListWatchersRequest(I &image_ctx,
                                            std::list<WatcherInfo> *watchers,
                                            Context *on_finish)
  : m_image_ctx(image_ctx), m_watchers(watchers), m_on_finish(on_finish),
  ...
  if (r == 0) {
    ...
    if (m_object_watchers.size() > 0) {
      ...
      for (auto &w : m_object_watchers) {
        ...
        bool mirroring = (it != m_mirror_watchers.end());
        bool me = (w.cookie == watch_handle); // XXXMG: does it work?
        m_watchers->emplace_back(w.addr, w.watcher_id, w.cookie,
                                 w.timeout_seconds, mirroring, me);
      }
    }
  }
