...
int Mirror<I>::group_create_snapshot(IoCtx& group_ioctx, const char *group_name,
  ...
  for (size_t i = 0; i < on_finishes.size(); i++) {
    image_snapshot_create(image_ctxs[i], flags, group_ioctx.get_id(), group_id,
                          group_snap_id, &snap_ids[i], &on_finishes[i]);
  }
...
int Mirror<I>::group_status_list(librados::IoCtx& io_ctx,
  ...
  map<std::string, cls::rbd::MirrorGroup> groups_internal;
  map<std::string, cls::rbd::MirrorGroupStatus> statuses_internal;
