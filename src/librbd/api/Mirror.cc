  ...
  for (const auto& image_id : image_ids) {
    ...
    std::list<uint64_t> snap_ids;
    {
      ...
      for (auto &it : img_ctx->snap_info) {
        ...
        if (info && info->mirror_peers.count(uuid)) {
    ...
    for (auto snap_id : snap_ids) {
      C_SaferCond cond;
      auto req = mirror_snapshot::UnlinkPeerRequest<I>::create(img_ctx, snap_id,
                                                               uuid, &cond);
      req->send();
      r = cond.wait();
      if (r == -ENOENT) {
...
int Mirror<I>::image_snapshot_create(I *ictx, uint64_t *snap_id) {
  ...
  auto req = mirror_snapshot::CreatePrimaryRequest<I>::create(
