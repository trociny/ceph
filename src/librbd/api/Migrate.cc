...
Migrate<I>::Migrate(I *src_image_ctx, librados::IoCtx& dst_io_ctx,
  ...
  : m_cct(static_cast<CephContext *>(dst_io_ctx.cct())),
    ...
    m_src_migration_spec(cls::rbd::MIGRATION_TYPE_SRC, m_dst_io_ctx.get_id(),
                         m_dst_image_name, m_dst_image_id, {}, 0, false) {
...
template <typename I>
int Migrate<I>::update_group(I *from_image_ctx, I *to_image_ctx) {
  group_info_t group_info;

  int r = librbd::api::Group<I>::image_get_group(from_image_ctx, &group_info);
  if (r < 0) {
  ...
  assert(!from_image_ctx->id.empty());

  ldout(m_cct, 10) << dendl;

  librados::Rados rados(from_image_ctx->md_ctx);
  IoCtx group_ioctx;
  rados.ioctx_create2(group_info.pool, group_ioctx);

  r = librbd::api::Group<I>::image_remove_by_id(group_ioctx,
                                                group_info.name.c_str(),
                                                from_image_ctx->md_ctx,
                                                from_image_ctx->id.c_str());
  if (r < 0) {
...
int Migrate<I>::abort(const std::string &dst_image_id, bool mirroring) {
  ...
  r = dst_image_ctx->state->open(OPEN_FLAG_IGNORE_MIGRATING);
  if (r < 0) {
    ldout(m_cct, 0) << "failed to open destination image: " << cpp_strerror(r)
                    << dendl;
  } else {
    ...
    ldout(m_cct, 10) << "updating group" << dendl;

    r = update_group(dst_image_ctx, m_src_image_ctx);
    if (r < 0) {
      return r;
    }
  }
