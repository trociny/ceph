...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::mirror_snapshot::UnlinkPeerRequest: " \
                           << this << " " << __func__ << ": "
...
void UnlinkPeerRequest<I>::unlink_peer() {
  ...
  if (info->mirror_peers.count(m_mirror_peer_uuid) == 0 ||
      info->mirror_peers.size() == 1U) {
    m_image_ctx->image_lock.unlock_shared();
    remove_snapshot();
    return;
  }
...
void UnlinkPeerRequest<I>::remove_snapshot() {
  ...
  if (info->mirror_peers.size() > 1 ||
      info->mirror_peers.count(m_mirror_peer_uuid) == 0) {
    finish(0);
    return;
  }
...
template class librbd::mirror_snapshot::UnlinkPeerRequest<librbd::ImageCtx>;
